...
function buildTypeDetailSegments(
  ...
  if (isTieredExpr && other.is_task) {
    ...
    if (isPerCall && modelPrice != null) {
      const finalPrice = modelPrice * (getGroupRatio(other) ?? 1)
      segments.push({
        text: `${t('Per-call')} · ${formatBillingCurrencyFromUSD(finalPrice, priceOpts)}`,
      })
    } else if (other.model_ratio != null) {
      const channelMultiplier = getGroupRatio(other) ?? 1
...
export function useCommonLogsColumns(
    ...
    columns.push(
...

      {
        ...
        cell: function DetailsCell({ row }) {
          ...
          const ip = canShowIp ? log.ip.trim() : ''
          ...
          return (
            <>
              <button
                type='button'
                className='group flex max-w-[200px] flex-col items-start gap-0.5 text-left text-xs'
  ...
  }, [t, isAdmin, isRoot, showBillingSource, currency, recordIpEnabled])
}
